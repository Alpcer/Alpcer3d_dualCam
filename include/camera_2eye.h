...
namespace CAMERA2EYE {
    ...
    private:
        ...
        CudaNvjpegDecoder* pDecoder = nullptr;
        FisheyeStereoPipelineGpu* pStereoPipelineGpu = nullptr;
    ...
    public:
        ...
        ~CameraInternal() {
            ...
            if (pStereoPipelineGpu != nullptr) {
                delete pStereoPipelineGpu;
                pStereoPipelineGpu = nullptr;
            }
        }
...
//                    long t = common_utils::currentTimeMilliseconds();
                    if (!pDecoder->decode(mptr[0], bytesused, frame)) {
...
//                    spdlog::info("image decoded, cost:{}s.\n", (common_utils::currentTimeMilliseconds()-t)/1000.0f);
        ...
        inline bool shootStereo(cv::Mat& left, cv::Mat& right, cv::Mat& parallax) {
            setAutoExposure(true, false);
            queueFrameBuffer();
            size_t bytesused = dequeueFrameBuffer();

            Mat frame;
            if (cur_width == width && pStereoPipelineGpu != nullptr) {
                pStereoPipelineGpu->processGpu(mptr[0], bytesused, left, right, parallax);
            } else {
                return false;
            }
            return true;
    ...
    private:
        ...
        void requestKernelSpace() {
            ...
            for(int i = 0; i <1;i++) {
                ...
                pDecoder = new CudaNvjpegDecoder(width, height);
                if (pStereoPipelineGpu != nullptr) {
                    delete pStereoPipelineGpu;
                    pStereoPipelineGpu = nullptr;
                }
...
//                    const std::string engine_path = "/IGEV-plusplus-main/pretrained_models/tensorrt/kitti_1024.engine";
                    const std::string engine_path = "/Fast-FoundationStereo-master/output/fast_foundationstereo512.engine";
                    pStereoPipelineGpu = new FisheyeStereoPipelineGpu(engine_path, width, height, k[0], d[0], k[1], d[1], CV_PI, CV_PI, cv::norm(T));
                }
            }
        }
    ...
    public:
        ...
        inline bool shootStereo(cv::Mat& left, cv::Mat& right, cv::Mat& parallax) {
            return cameraInternal->shootStereo(left, right, parallax);
        }
